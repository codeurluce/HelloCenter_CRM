...
router.get('/today-summary', filesControllers.getNewFilesCountByAgent); // Récupère le résumé des nouvelles fiches assignées au agent connecté pour la journée en cours // Méthode : GET /api/files/today-summary
router.get('/admin-summary', filesControllers.getAdminFilesSummary); // Récupère le résumé global des nouvelles fiches pour l'administration // Méthode : GET /api/files/admin-summary
router.get('/files-by-agents', filesControllers.getFilesbyAgent); // Récupère toutes les fiches assignées à chaque agent (par agent)        // Méthode : GET /api/files/files_by_agents
...
router.put('/assigned_To', filesControllers.getAssignedFichesTo); // Récupère les fiches assignées à l'agent connecté (ou autre selon le corps de requête) // Méthode : PUT /api/files/assigned_To
router.put('/import_files', filesControllers.importFiles); // Importe des fiches dans la base de données à partir d'une source externe ou fichier // Méthode : PUT /api/files/import_files
router.put('/unassign', filesControllers.unassignFiches) // Désassigne une fiche déja assigner a un agent et remet la fiche dans le statut nouvelle
router.delete('/:id/delete', filesControllers.deleteFile) // Supprime une fiche spécifique via son ID // Méthode : DELETE /api/files/:id/delete
router.delete('/delete-batch', filesControllers.deleteFilesBatch) // Supprime plusieurs fiches via leurs IDs // Méthode : DELETE /api/files/:id/delete

...
import { BellRing, CheckCircle, Clock, AlertTriangle, X } from "lucide-react";
import { useNotifications } from "./NotificationsContext";

export default function NotificationsFinContrat({ loading = false }) {
  const { finContratNotifs, markAsRead } = useNotifications(); // ✅ utiliser le context
  ...
  const getIcon = (jours) => {
    ...
    if (jours <= 10) return <Clock className="text-orange-500" size={20} />;
    if (jours <= 30) return <CheckCircle className="text-blue-500" size={20} />;
        ...
        {loading ? (
          ...
          filtered.map((notif) => (
            <div
              ...
              className={`flex justify-between items-center p-4 rounded-xl shadow-sm border cursor-pointer transition ${
                !notif.lu
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl w-[400px] p-6 relative animate-fadeIn">
            ...
            <div className="text-sm text-gray-700 space-y-2">
              ...
              <p><strong>Date de fin :</strong> {selectedNotif.date_fin}</p>
              <p>
                <strong>Temps restant :</strong>{" "}
                {selectedNotif.jours_restant > 0
                  ? `${selectedNotif.jours_restant} jours`
                  : "Contrat expiré"}
              </p>

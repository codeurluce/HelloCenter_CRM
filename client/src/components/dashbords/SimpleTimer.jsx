...
const SimpleTimer = ({
    ...
    const handleStatusChange = async (newStatusKey, pause = null) => {
        const newStatusFr =
            Object.entries(statusKeyMap).find(([, key]) => key === newStatusKey)?.[0] || newStatusKey;
            ...
            <div className="flex items-center gap-4">
                {Object.entries(STATUS_ICONS).map(([key, { icon: Icon, label, color }]) => {
                    ...
                    return (
                        ...
                        >
                            <Icon
                                className={`text-xl transition-all ${isActive ? color : 'text-gray-400'} ${isActive ? 'scale-125' : 'opacity-70'
                                    }`}
                            />
